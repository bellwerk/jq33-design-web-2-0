...
- **Smallest third correction:** preserve the exact font outlines, weights, text, and layout while reducing graph bytes. Generate a `17960`-byte route-only subset from the existing `48256`-byte variable Inter source for the static home panel, retain `swap`, and preload only that subset. Inline the existing `4016`-byte commercial H1 subset and remove its redundant external preload. The full shared fonts remain byte-identical and available elsewhere.
- **Local correction proof:** the combined browser contract passes: the home subset loads once with no full Inter request, and the service H1 uses the embedded exact face with no external request. Three fresh local mobile captures measured home LCP `1828.1ms` median and service LCP `2450.8ms` median; both performance scores were at least `96`, TBT at most `46ms`, and CLS at most `0.07`.
